...
bool ClsValidate::isNumberBetween(double Number, double From, double To) {
    if (Number <= To && Number >= From) {
        return 1;
    }
    else {
...
bool ClsValidate::isNumberBetween(float Number, float From, float To) {
    if (Number <= To && Number >= From) {
        return 1;
    }
    else {
...
float ClsValidate::readFloatNumber() {
    float number;
    while (!(cin >> number)) {
        cin.clear();
        cin.ignore(numeric_limits<std::streamsize>::max(), '\n');
        cout << "Invalid Float, Pls enter agian!\n";
    }
    return number;
}
float ClsValidate::readFloatNumberBetween(float From, float To, string ErrorMessage) {
