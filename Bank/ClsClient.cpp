...
bool ClsClient::transfer(float Amount, ClsClient& DestinationClient,string UserName) {
    if (Amount > _balance) {
        return false;
    }
    withdraw(Amount);
...
void ClsClient::_registerTransferLog(float Amount, ClsClient DestinationClient, string UserName) {
    string strDataLine = _prepareTrasferLogRecord(Amount, DestinationClient, UserName);
    fstream myFile;
...
string ClsClient::_prepareTrasferLogRecord(float Amount, ClsClient DestinationClient, string UserName,string Seperator) {
    ...
    TransferLogRecord += to_string(_balance) + Seperator;
    TransferLogRecord += to_string(DestinationClient.getBalance()) + Seperator;
